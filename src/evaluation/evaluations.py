...
from src.evaluation.test_metrics import cacf_torch, HistoLoss, CovLoss
...
def fake_loader(generator, num_samples, batch_size, config, **kwargs):
...
def full_evaluation(generator, real_train_dl, real_test_dl, config, **kwargs):
    ...
    for i in tqdm(range(5)):
        ...
        fake_train_dl = fake_loader(generator, num_samples=8000, batch_size=128, config=config)
        fake_test_dl = fake_loader(generator, num_samples=2000, batch_size=128, config=config
                                   )
        ...
        hist_losses.append(to_numpy(HistoLoss(real.unsqueeze(1), n_bins=50, name='marginal_distribution')(fake.unsqueeze(1))))
        cov_losses.append(to_numpy(CovLoss(real.unsqueeze(1), name='covariance')(fake.unsqueeze(1))))
        # acf_losses.append(to_numpy(ACFLoss(real, name='auto_correlation')(fake)))
        # FIDs.append(predictive_fid)
        # KIDs.append(predictive_kid)
    
    # Histogram plots
    histogram_plot(real, fake, config)
